...
Built 2026-09-08/09 (`bridge/` and `cli/` in this repository),
verified live on Windows 11 against Studio 21.1.0.14 with a disposable `_ref_scratch` project. Tags: [measured] unless
...
A driver script ran the phases project → media → timeline → color → render → more →
...
fcpxml and edl. Outputs written to a scratch folder
...
catalog (`cli/tools/make_spec.py catalog.json spec.yaml`, where `catalog.json` is the output of
`GET /v1/commands`; re-run it after any bridge change and regenerate, so the CLI cannot drift
from the bridge. The derived spec ships as `cli/spec.yaml`). 11 resources (`system project
...
reach a sidecar on another machine with an SSH local-forward. Shipcheck 2026-09-09 (run 6): 7/7 legs, scorecard 93/100
...
**Where it lives:** `cli/` in this repository (source + `spec.yaml` + `SKILL.md`;
`go build ./cmd/davinci-resolve-pp-cli` yields the binary). History DB
...
- The media-pool fixture is not durable: something between sessions emptied `_ref_scratch`'s
  bins. Fixture recipe (all through the sidecar): `import-media` `<scratch>/ref_a.mp4`;
...
**Generator fixes this CLI forced (in CLI Printing Press, 2026-09-09):**
