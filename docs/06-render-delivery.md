...
- For a 30-s vertical short on a mid-range NVIDIA GPU expect well under a minute wall time with NVENC; the 1-s Solid Color smoke took "tens of seconds" end-to-end because queueing dominates.
...
4. Frame sanity: extract 3 frames (first, middle, last) + a contact sheet; OCR burned captions; for a real deliverable, watch it and transcribe it (dead air, repeated phrases).
...
Resolve's remote-render workers need a shared PostgreSQL project library; a Disk database doesn't support it. If a second render node is needed without PostgreSQL, the offline-authoring + `ImportTimelineFromFile`/`ImportProject` route on the second machine is the realistic path, or FFmpeg for non-Resolve encodes.
