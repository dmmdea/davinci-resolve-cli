...
## Text+ (TextPlus) — the workhorse for titles [MEASURED 2026-09-01, 21.0.4.5 — see the verified block after the sample]
```python
...
### Verified Text+ facts [measured 2026-09-01, Studio 21.0.4.5, title inserted by `InsertFusionTitleIntoTimeline("Text+")`]
...
- `GetInputList()` returns **309** inputs keyed 1..309 (NOT by ID); read values with `tp.GetInput("<ID>")`, never `GetInput(k)` with the numeric key (returns None). Full ID list: `resolve-21.0.4-settings-dump.json` → `textplus_input_ids`.
...
## Color
- LUT on a node: `g = item.GetNodeGraph(); g.SetLUT(1, "MyLooks/Look A.cube")` (path relative to the LUT root or absolute; `project.RefreshLUTList()` after copying a new cube). `g.GetLUT(1)` reads back.
...
- Color management: set `colorScienceMode` first, then `colorSpaceInput/Timeline/Output`; ACES contexts lock custom spaces. A common YouTube default = DaVinci YRGB, Rec.709 Gamma 2.4, LUT look.
...
- A practical split: do the audio chain in FFmpeg upstream (VO processing → mix → two-pass loudnorm) and let Resolve only place finished stems. Keep music/VO/SFX on separate audio tracks (A1 VO, A2 music, A3 SFX) for the editor's benefit even when mixing upstream.
...
- For karaoke-grade word timing, an external STT with hotword/vocabulary support (on a ≤30 s stem) is still the more controllable source; Resolve 21.1's transcript is a real cross-check (09 §5).
