...
8. Grades: `item.GetNodeGraph().SetLUT(1, "MyLooks/Look A.cube")` (or `SetCDL`, `ApplyGradeFromDRX`).
...
| "remove silence / dead air" | pacing cut | STT (e.g. Whisper word timestamps) → silence intervals → generate MANY clipInfos (each kept speech span = one clip); keep 120–200 ms pads, ≥500–750 ms min silence; place contiguous |
...
| "match the audio / sync" | `mp.AutoSyncAudio([...], {waveform})` — content dependent; fallback = STT + cross-correlation to compute the offset, then place with computed record |
...
- Proxies/optimized media: `LinkProxyMedia(path)` per clip; project `perfProxyMediaMode` 1/2; on an 8 GB GPU prefer proxies for 4K/5.7K sources.
