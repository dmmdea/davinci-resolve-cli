...
| # | Symptom | Cause | Fix |
|---|---|---|---|
| 1 | Every write returns None/False; reads fine; `status.page == null` [measured ×2 machines] | Resolve on the Project Manager / no project page | `load-project NAME --yes` (works from page-null), then retry. Zero projects in DB → `CreateProject` from script also works from page-null [measured 2026-09-01]; or run Resolve `-nogui` (comes up on page `media`, never null) |
...
| 3 | `import DaVinciResolveScript` crashes 0xC0000005 in `PyInit_fusionscript` [measured on 21.0.4] | wrong CPython ABI for this machine (machine A needed 3.12.10, machine B 3.14.7). **Not reproduced on 21.1**: 3.11/3.14 system, uv and the bundled `ResolvePython.exe` all bind (09 §2) | run `_probe.py` under candidates; pin via `deploy.ps1 -PythonPin` (21.1: prefer `ResolvePython.exe`); scrub PYTHONHOME/PYTHONPATH |
...
| 19 | `SetLUT` False | LUT not discovered / path wrong / node index 0 | copy to the LUT root (`…\Support\LUT\<YourFolder>\`), `project.RefreshLUTList()`, path relative to root, 1-based node |
...
| 25 | Render OK but "GPU memory full" mid-way on an 8 GB GPU | 8 GB VRAM: NR, Magic Mask, 4K, Fusion | lower timeline res, cache heavy items, drop NR, free VRAM held by other GPU apps, render in ranges |
...
| 37 | Word timestamps drift on long files (external STT) | STT alignment | captions from a ≤30 s stem; assert the alignment model matches the language |
| 38 | Renders land in a synced cloud folder and upload mid-write | the sync client grabs a partial file | render to a local, unsynced folder; copy out after `verify-render` passes |
...
| 44 | `MediaStorage.GetFileList/GetSubFolderList/AddItemListToMediaPool/StartCloneMedia` return `[]`/False [21.1] | forward-slash paths; the log says `no mapped volume found for path X:/…` | pass backslash paths (`os.path.normpath`) |
...
2. Is Resolve the Studio process in the console session (`Get-Process Resolve | select SessionId`)? Any modal on screen (ask, or check a screenshot if remote capture is available)?
