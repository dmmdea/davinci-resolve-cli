# DaVinci Resolve Printed CLI Agent Guide

This directory is a generated `davinci-resolve-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
davinci-resolve-pp-cli doctor --json
davinci-resolve-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
davinci-resolve-pp-cli which "<capability>" --json
davinci-resolve-pp-cli <command> --help
```

Add `--agent` to command invocations for JSON, compact output, non-interactive defaults, and no color:

```bash
davinci-resolve-pp-cli <command> --agent
```
...
```bash
davinci-resolve-pp-cli <command> --help
davinci-resolve-pp-cli <command> --dry-run --agent
```
