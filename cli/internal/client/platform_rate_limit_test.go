...
import (
	...
	"github.com/dmmdea/davinci-resolve-cli/cli/internal/config"
	"github.com/dmmdea/davinci-resolve-cli/cli/internal/platform"
)
...
func platformRateLimitTestClient(t *testing.T, server *httptest.Server) (*Client, *platform.Session) {
	...
	session, err := platform.PrepareProfileSourceFromConfig(&platform.Profile{
		...
		Sources: map[string]platform.SourceProfile{
			"davinci-resolve": {CredentialRef: "fixture://Test/Synthetic/token", ExpectedAccountID: "acct-a", ExpectedOrgName: "Tenant A", ExpectedStoreID: "store-a", ExpectedStoreDomain: "tenant-a.example.test", ExpectedPropertyID: "123", ExpectedBaseURL: "https://tenant-a.example.test"},
		},
	}, "davinci-resolve-pp-cli", "davinci-resolve")
	if err != nil {
		t.Fatal(err)
	}
