...
Resource scoping:
  ...
  davinci-resolve-pp-cli sync
  # Sync specific resources only
  davinci-resolve-pp-cli sync --resources item,project

  # Full resync (ignore previous checkpoint)
  davinci-resolve-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  davinci-resolve-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  davinci-resolve-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  davinci-resolve-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"item":     "/v1/items",
		"project":  "/v1/projects",
		"storage":  "/v1/storage-browse",
		"system":   "/v1/presets",
		"timeline": "/v1/timelines",
	}
