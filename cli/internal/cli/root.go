...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "davinci-resolve-pp-cli",
		Short: `Every documented DaVinci Resolve scripting operation, plus the cross-project history and doctor diagnostics that Resolve itself — and every other Resolve CLI or MCP server — throws away.`,
		Long: `Every documented DaVinci Resolve scripting operation, plus the cross-project history and doctor diagnostics that Resolve itself — and every other Resolve CLI or MCP server — throws away.
...
Health check: run 'davinci-resolve-pp-cli doctor' to verify connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("davinci-resolve-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'davinci-resolve-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAudioCmd(flags))
	rootCmd.AddCommand(newClipCmd(flags))
	rootCmd.AddCommand(newColorCmd(flags))
	rootCmd.AddCommand(newFusionCmd(flags))
	rootCmd.AddCommand(newItemCmd(flags))
	rootCmd.AddCommand(newPoolCmd(flags))
	rootCmd.AddCommand(newProjectCmd(flags))
	rootCmd.AddCommand(newRenderCmd(flags))
	rootCmd.AddCommand(newStorageCmd(flags))
	...
	if parent, _, err := rootCmd.Find(strings.Fields("doctor")); err == nil {
		addNovelCommandIfAbsent(parent, newNovelDoctorHistoryCmd(flags))
	}
	addNovelCommandIfAbsent(rootCmd, newNovelMarkerCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRunsCmd(flags))
