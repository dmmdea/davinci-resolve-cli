...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  davinci-resolve-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  davinci-resolve-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  davinci-resolve-pp-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"item", "project", "system", "timeline"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  davinci-resolve-pp-cli workflow status

  # Show status as JSON
  davinci-resolve-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("davinci-resolve-pp-cli")
			}
