...
var whichIndex = []whichEntry{
	{Command: "runs list", Description: "See exactly which commands this CLI has run, whether each succeeded, and whether Resolve was too busy and had to be retried.", Group: "Local run history", WhyItMatters: "Reach for this first when diagnosing any pipeline hiccup instead of re-deriving state from a terminal scrollback."},
	{Command: "marker search", Description: "Find a marker by text across every project this CLI has touched, not just the one currently open in Resolve.", Group: "Cross-project marker index", WhyItMatters: "Use when the user references a marker/note from an unspecified or past project."},
	{Command: "render history", Description: "See every render this CLI has ever queued across all projects, with the actual output file verified against the requested spec.", Group: "Render delivery history", WhyItMatters: "Use for any delivery-audit or 'did this actually render correctly' question spanning more than the current session."},
	{Command: "color audit", Description: "See which timelines are still on an old color grade version across your whole project library, not just the one open in Resolve.", Group: "Grade-version audit", WhyItMatters: "Use before a client delivery to confirm every episode/timeline carries the latest approved grade."},
	{Command: "fusion titles search", Description: "Find what text was used in a past Text+/title, across every project, without reopening it.", Group: "Title text catalog", WhyItMatters: "Use when the user asks what a past title/lower-third said instead of reopening that project."},
	{Command: "timeline log", Description: "See the full append/delete history for one timeline — the only record of what actually changed, since Resolve has no undo API.", Group: "Timeline operation trail", WhyItMatters: "Use to audit or explain what an automated build/edit pass actually did to a timeline."},
	{Command: "doctor history", Description: "See whether this machine's Resolve/Python environment has drifted since yesterday, not just whether it's healthy right now.", Group: "Environment drift history", WhyItMatters: "Use first when an unattended pipeline starts failing intermittently on one machine but not another."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  davinci-resolve-pp-cli which "stale tickets"
  davinci-resolve-pp-cli which "bottleneck"
  davinci-resolve-pp-cli which --limit 1 "send message"
  davinci-resolve-pp-cli which                                # list the full capability index`,
