...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DAVINCI_RESOLVE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DAVINCI_RESOLVE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/dmmdea/davinci-resolve-cli/cli/feedback")
...
When ` + "`DAVINCI_RESOLVE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DAVINCI_RESOLVE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  davinci-resolve-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  davinci-resolve-pp-cli feedback --stdin < notes.txt
  davinci-resolve-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  davinci-resolve-pp-cli feedback list
  davinci-resolve-pp-cli feedback list --limit 5
  davinci-resolve-pp-cli feedback list --json`,
